...
{ // unnamed pipes + classificador
    ...
    if (putente == NULL){
        printf("\nErro a alocar memoria para estrutura UTENTE!\n");
        free(putente);
    }

    pbalcao = malloc(sizeof(balcao));
    if (pbalcao == NULL){
        printf("\nErro a alocar memoria para estrutura BALCAO!\n");
        free(pbalcao);
    }
    ...
    if (id > 0)
    {
        ...
        close(FD_BC[0]);
        write(FD_BC[1], pbalcao->sintomas, 50);
        close(FD_CB[1]);
        read(FD_CB[0], putente->recebe_class, 50);
        close(FD_CB[0]);

        printf("recebeu do classificador: %s\n", putente->recebe_class);
    }
...
{ //variaveis de ambiente
    ...
    {
        b.quantidadeClientes = atoi(getenv("MAXCLIENTES"));
    }
    else
    {
    ...
    {
        b.quantidadeMedicos = atoi(getenv("MAXMEDICOS"));
    }
    else
    {
...
int main()
{
    ...
    //Ler
    if (read(fifo, pbalcao->sintomas, 50) == -1)
    {
        printf("Erro [Read - Fifo balcao - Sintomas]\n");
        exit(EXIT_FAILURE);
    }

    printf("Sintomas recebidos: %s\n", pbalcao->sintomas);

    close(fifo);

    fifoUte = open(FIFO_UTE, O_WRONLY);
    ...
    while (1)
    {
        ...
        if(write(fifoUte, putente->envia_para_class, 50) == -1)
        {
            printf("Erro [Write - Fifo Utente - Sintomas ]\n");
            //exit(EXIT_FAILURE);
        }
